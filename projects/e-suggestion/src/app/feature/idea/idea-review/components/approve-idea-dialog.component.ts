...
@Component({
  selector: 'ba-approve-idea-dialog',
  standalone: true,
  template: `
    <ba-base-dialog (cancel)="cancel()">
      <div class="w-full flex flex-col gap-y-3 p-4 items-center ">
        ...
        <span class="text-[1em] font-semibold">You're about to approve the idea with id {{ data.ideaId }}</span>
        <span class="text-[1em]"
...
export class ApproveIdeaDialogComponent {
  private readonly dialogRef = inject(MatDialogRef<ApproveIdeaDialogComponent>);
